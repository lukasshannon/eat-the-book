...
- Turn book interaction requirement (2026-05-17): prevent browser selection, callout/context menu, and asset dragging during `docs/book.html` page-turn holds/drags so touches feel like manipulating a physical page rather than selecting/copying content.
- Turn book fidelity requirement (2026-07-04): match turn.js interaction quality in the `turn-book` component — soft pages must fold along a real crease (reflected-corner geometry) showing back-face page content with crease-tracking gradients, for drags, programmatic turns, and corner hover peels alike; hard covers may keep the rigid flip, and reduced-motion mode falls back to the rigid path.
